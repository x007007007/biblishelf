...
def portable_static(request, db_key, path):
    repo = RepoConfigModel.get_repo_path_map()[db_key]
    return static.serve(request, path=path, document_root=repo.path)


def portable_path(request, db_key, path_id):
    repo = RepoConfigModel.get_repo_path_map()[db_key]
    if not (path_obj := PathModel.objects.using(db_key).filter(id=path_id).first()):
        path_obj.is_exist = False
        path_obj.save(update_fields=('is_exist',), using=db_key)
